...
export default function DiscountForm({ product, onSubmit }) {
  ...
  const [discountPercent, setDiscountPercent] = useState(product ? product.discountPercent : '');
  ...
  const submitHandler = async (e) => {
    ...
    if (!price || !description || !productName || !quantity || !selectedCategory || !discountPercent) {
    ...
    try {
      ...
      const res = await axios.post('/api/discount', { productName,
          price,
          quantity,
          description,
          selectedCategory,
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-4">
              ...
              <div className="flex-grow">
                ...
                <input type="number" placeholder="Discount Percent" 
                  className="w-full p-2 border border-gray-300 rounded" value={discountPercent} onChange={(e) => setDiscountPercent(e.target.value)} />
              </div>
